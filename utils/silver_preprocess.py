...
def clean_lms(spark, input_path, output_path, date):
    for folder in os.listdir(input_path):
        if os.path.isdir(os.path.join(input_path, folder)) and (date is None or folder == date):
            lms = spark.read.parquet(os.path.join(input_path, folder))
            lms = lms.withColumn('tenure', col('tenure').cast('integer'))
            lms = lms.withColumn('installment_num', col('installment_num').cast('integer'))
            lms = lms.withColumn('loan_amt', col('loan_amt').cast('double'))
            lms = lms.filter(lms['tenure'] == lms['installment_num'])
            lms.write.mode('overwrite').parquet(os.path.join(output_path, folder))
...
def change_dtype_fc(spark, input_path, output_path, date):
    for folder in os.listdir(input_path):
        if os.path.isdir(os.path.join(input_path, folder)) and (date is None or folder == date):
            fc = spark.read.parquet(os.path.join(input_path, folder))
            fe_cols = [c for c in fc.columns if c.startswith('fe_')]
            for c in fe_cols:
                fc = fc.withColumn(c, col(c).cast('integer'))
            fc.write.mode('overwrite').parquet(os.path.join(output_path, folder))
...
def process_silver_tables(spark, input_path, output_path, date):
    ...
    change_dtype_fc(spark, os.path.join(input_path, 'feature_clickstream'), os.path.join(output_path, 'feature_clickstream'), date)